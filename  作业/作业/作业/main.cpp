...
int main()
{
    ...
    int score[140];                                 //存放裁判打分；
    string name[7];                                 //存放裁判姓名；
    string competitor[40];                          //选手信息；
    ...
    for(i=0;i<20;i++)                                //排名需要的数；
    {
        rank[i]=i+1;
    }
    for(i=0;i<20;i++)                                //为学生编号;
    {
        number[i]=i;
    }
    for(i=0;i<147;i++)                               //将数据放入数组；
    {
       if((i+1)%21==0)                               //将裁判名放入对应数组；
       {
           j=(i+1)/21-1;
           judgmentin>>name[j];
       } else                                        //将打分放入对应数组；
    ...
    for(i=0;i<40;i++)                                 //将选手数据存入数组；
    {
        competitorin>>competitor[i];
    }
    for(a=0;a<20;a++)                                  //进行冒泡排序并且选手信息与分数绑定；
    {
        for(j=0;j<20;j++)
        {
